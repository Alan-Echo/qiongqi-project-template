...
import com.qiongqi.modules.user.model.vo.VUserVo;
...
public class AppletLoginController {
     ...
     */
    @PostMapping("login")
    public ResponseData<VUserVo> login(@RequestBody JSONObject userInfo){
        ...
        VUserDTO userDTO = vUserService.getByOpenId(openid); //根据openId查询数据库中是否有这个用户
        VUserEntity entity =null;
        if(userDTO==null){//没有当前登录用户
            userDTO = new VUserDTO();
        }
        //设置新数据
        userDTO.setAvatar(userInfo.getString("avatarUrl"));
        userDTO.setUserName(userInfo.getString("nickName"));
        userDTO.setOpenId(openid);
        if(userDTO.getId()==null){
            entity = vUserService.save(userDTO);//将数据存进数据库
        }else {
            vUserService.update(userDTO);
            entity = ConvertUtils.sourceToTarget(userDTO,VUserEntity.class);
        }
        //将权限/角色放入token中
...
//        map.put("nickName",entity.getNickName());
        ...
        try {
            ...
            return new ResponseData<VUserVo>().success("登录成功",ConvertUtils.sourceToTarget(userDTO, VUserVo.class));
        } catch (Exception e) {
            e.printStackTrace();
        }
